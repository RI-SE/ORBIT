"""Tests for orbit_core.utils.logging_config module."""

import logging

from orbit_core.utils.logging_config import get_logger, setup_logging
...
class TestGetLogger:
    ...
    def test_get_logger_with_orbit_prefix(self):
        ...
        logger = get_logger('orbit_core.models.project')

        assert logger.name == 'orbit_core.models.project'
        assert isinstance(logger, logging.Logger)

    def test_get_logger_without_namespace_prefix(self):
        """A bare name lands in the default namespace."""
        logger = get_logger('my_module')

        assert logger.name == 'orbit_core.my_module'

    def test_get_logger_keeps_gui_namespace(self):
        """Names already under a configured namespace are left alone."""
        assert get_logger('orbit.gui.main_window').name == 'orbit.gui.main_window'
    ...
    def test_get_logger_with_dunder_name(self):
        ...
        assert logger.name == 'orbit_core.models.road'
...
class TestLoggingIntegration:
    ...
    def test_child_logger_inherits_level(self):
        ...
        child_logger = get_logger('orbit_core.models')
    ...
    def test_logging_with_different_modules(self, tmp_path):
        ...
        # Flush
        for handler in logging.getLogger('orbit_core').handlers:
            handler.flush()

        content = log_file.read_text()

        assert "orbit_core.module1" in content
        assert "orbit_core.module2" in content
