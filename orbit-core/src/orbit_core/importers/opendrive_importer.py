...
from orbit_core.models import ControlPoint, Junction, ParkingSpace, Polyline, Project, Road, RoadObject, Signal
...
from orbit_core.models.lane import LaneType as ORBITLaneType
...
from orbit_core.models.polyline import LineType, RoadMarkType
...
class OpenDriveImporter:
    ...
    def _import_road(self, odr_road: ODRRoad, options: ImportOptions) -> Optional[Dict]:
        ...
        if odr_road.lane_sections:
            ...
            road.lane_info.left_count = sum(1 for lane in first_section.lanes if lane.id > 0)
            road.lane_info.right_count = sum(1 for lane in first_section.lanes if lane.id < 0)
