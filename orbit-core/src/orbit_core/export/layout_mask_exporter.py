...
try:
    import cv2
except ImportError as _e:  # pragma: no cover - depends on install extras
    raise ImportError(
        "Layout mask export requires OpenCV. Install it with: pip install 'orbit-core[imagery]'"
    ) from _e
import numpy as np
...
class LayoutMaskExporter:
    ...
    def _collect_opendrive_polygons(self) -> List[LanePolygonData]:
        ...
        from orbit_core.export.curve_fitting import CurveFitter
